...
<div class="container-fluid">
	...
	<div class="col-lg-12">
		<div class="row">
			<!-- FORM Panel -->
			<div class="col-md-4">
			<form action="" id="manage-therapists">
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control" name="name_pref" placeholder="(Therapists)" required="">
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								...
								<select name="specialty_ids[]" id="" multiple=""  class="custom-select browser-default select2">
									<option value=""></option>
									<?php 
									$qry = $conn->query("SELECT * FROM specialty order by name asc");
					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$cats = $conn->query("SELECT * FROM therapists_list order by id asc");
								...
								<tr>
									...
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit-therapists" type="button" data-id="<?php echo $row['id'] ?>" data-name="<?php echo $row['name'] ?>" data-name_pref="<?php echo $row['name_pref'] ?>" data-clinic_address="<?php echo $row['clinic_address'] ?>" data-contact="<?php echo $row['contact'] ?>"  data-img_path="<?php echo $row['img_path'] ?>" data-specialty_ids="<?php echo $row['specialty_ids'] ?>" data-email="<?php echo $row['email'] ?>">Edit</button>
										<button class="btn btn-sm btn-danger delete_therapists" type="button" data-id="<?php echo $row['id'] ?>">Delete</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	function _reset(){
		$('[name="id"]').val('');
		$('#manage-therapists').get(0).reset();
	}
	$('table').dataTable()
	$('#manage-therapists').submit(function(e){
		e.preventDefault()
		start_load()
		$('#msg').html('')
		$.ajax({
			url:'ajax.php?action=save_therapists',
	...
	$('.edit-therapists').click(function(){
		start_load()
		var cat = $('#manage-therapists')
		cat.get(0).reset()
	...
	$('.view_schedule').click(function(){
		uni_modal($(this).attr('data-name')+" - Schedule","view_therapists_schedule.php?id="+$(this).attr('data-id'))
	})
	$('.delete_therapists').click(function(){
		_conf("Are you sure to delete this therapists?","delete_therapists",[$(this).attr('data-id')])
	})
	
	function delete_therapists($id){
		start_load()
		$.ajax({
			url:'ajax.php?action=delete_therapists',
			method:'POST',
			data:{id:$id},
			success:function(resp){
				if(resp==1){
